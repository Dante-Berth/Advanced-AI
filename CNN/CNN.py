...
from tensorflow.keras.layers import Dense, Flatten, MaxPooling1D, Dropout, BatchNormalization, Input, Conv1D, \
    AveragePooling1D, Concatenate, LSTM, SimpleRNN, LSTM, Bidirectional, TimeDistributed, ConvLSTM2D, Concatenate, Dropout
...
class CNN_layer(tf.keras.layers.Layers):
    ...
    def __init__(self,filters,kernel_size,activation,pool_size,strides,pooling_layer):
        self.filters = filters
        self.kernel_size = kernel_size
        self.activation = activation
        self.pool_size = pool_size
        self.strides = strides
        if pooling_layer in ["AveragePooling1D,MaxPooling1D","AveragePooling2D","MaxPooling2D"]:
            self.pooling_layer = getattr(tensorflow.keras.layers, pooling_layer)(self.pool_size,self.strides,padding="same",
                                                                             data_format="channels_last")
        elif pooling_layer == "MetaPooling":
            self.pooling_layer = MetaPoolinglayer(self.pool_size,self.strides)
        else:
            self.pooling_layer = tf.nn.Identity()
        def call(self,inputs):
...
def loop_CNN_layer(first_input,n_layers,trial,name_layer):
    ...
    while i <n_layers:
        ...
        pooling_layer = trial.suggest_categorical(f'{name_layer}_CNN_pooling_layer_{i+1}', ['AveragePooling1D','MaxPooling1D'])
        ...
        CNN_OUTPUT[i+1] = CNN_POOLING_layer(CNN_OUTPUT[i],filters,kernel_size,activation,pool_size,stride,pooling_layer)
        i = i+1
        last_rank = i
    return CNN_OUTPUT