...
class ReductionLayerPooling(tf.keras.layers.Layer):
    def __init__(self, pool_size: int, strides: int, pooling_layer_name: str):
    ...
    def build(self, input_shape):
        ...
        input_dim_2 = input_shape[2]

        self.pool_size = input_shape[1]*self.ratio_pool_size*0.1//1
        self.strides = input_shape[2]*self.ratio_strides*0.1//1
    ...
    def call(self, input):
        ...
        return self.pooling_layer(input)
if __name__ == "__main__":
    tensor_3 = tf.random.uniform((12, 24, 36))
    tensor_4 = tf.random.uniform((12, 24, 36, 48))
    tensor_5 = tf.random.uniform((12, 6, 6, 1))
    tensor_6 = tf.random.uniform((12, 2, 2))
    ...
    #print(ReductionLayerPooling(3, 3, "AveragePooling1D")(tensor_4).shape)
    #print(ReductionLayerPooling(3, 3, "MetaPoolingLayer")([tensor_3, tensor_3]).shape)
    #print(ReductionLayerPooling(18, 20, "MetaPoolingLayer")([tensor_3, tensor_3]).shape)
    print(tf.keras.layers.MaxPool2D(6,6)(tensor_4).shape)
    print(tf.keras.layers.MaxPool2D(6, 6)(tensor_5).shape)
    print(tf.keras.layers.MaxPool1D(3, 5)(tensor_6).shape)
